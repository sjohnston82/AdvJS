...
const upperCaseNames = (people) => people.map((p) => p.name.toUpperCase());
...
const userLinks = (users) =>
  users.map(
    (user) => `<a href=\"/users/${user.username}\">${user.username}</a>`
  );
...
const toBeInterviewed = applicants.filter(
  (applicant) => applicant.advancedJSCourse
);
...
const getApplicantEmails = (applicants) =>
  applicants
    .filter((applicant) => applicant.age > 17)
    .filter((applicant) => applicant.advancedJSCourse)
...
const numberOfAdvancedGrads = applicants.reduce(
  (accum, applicant) => (applicant.advancedJSCourse ? accum + 1 : accum),
  0
);
...
const numberOfAdvancedGrads2 = applicants.filter(
  (applicant) => applicant.advancedJSCourse
).length;
...
// Hint: pass in an empty object for the default value of the accumulator.
//
